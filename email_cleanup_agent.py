...
def email_analytics():
    ...
    email_volume = pd.DataFrame({
        'Date': dates,
        'Received': [20 + int(10 * abs(hash(str(d)) % 100) / 100) for d in dates],
    ...
    fig = px.line(email_volume, x='Date', y=['Received', 'Sent'], 
                  ...
                  color_discrete_map={'Received': '#667eea', 'Sent': '#764ba2'})
    fig.update_layout(
        plot_bgcolor="rgba(0,0,0,0)",
        paper_bgcolor="rgba(0,0,0,0)",
    ...
    with col1:
        senders_data = {
            'Sender': ['Amazon', 'Google', 'LinkedIn', 'GitHub', 'Netflix', 'Others'],
            'Count': [45, 32, 28, 23, 15, 67]
        }
        fig_pie = px.pie(pd.DataFrame(senders_data), values='Count', names='Sender', 
    ...
    with col2:
        categories_data = {
            'Category': ['Promotions', 'Work', 'Personal', 'Bills', 'Social'],
            'Percentage': [40, 25, 20, 10, 5]
        }
        fig_bar = px.bar(pd.DataFrame(categories_data), x='Category', y='Percentage',
...
def main():
    ...
    if "Dashboard" in current_page:
        ...
        with col1:
            keep_important = st.checkbox("Keep flagged emails", value=True)
            backup_before = st.checkbox("Backup before cleanup", value=True)
